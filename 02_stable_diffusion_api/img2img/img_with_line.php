...
function getPayload($img_name,$model_name,$seed_fixed,$p,$np){
	...
	//$payload["init_images"]= [getImageBase64("data/img2line/".$img_name)];
//$payload["image_mask"]= getImageBase64("data/img_src/".$img_name);
	$payload["init_images"]= [getImageBase64("data/img_src/".$img_name)];
$payload["image_mask"]= getImageBase64("data/img2line/".$img_name);
...
function post_sd_api($payload,$img_name){
	...
	foreach ($arr_res_images as $k=>$v) {
		$img_dir = "data/img_dst_line/".$img_name;
		if(!is_dir($img_dir)){
			mkdir($img_dir,0777);
		}
...
$img_list = scandir("data/img2line");
sort($img_list);
